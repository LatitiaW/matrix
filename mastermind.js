...
function MastermindComponent(params) { 
  var self = this,
    ...
    colors = ['red', 'blue', 'white', '#0F0', 'black', 'yellow'],
  ...
  _.extend(self, { // properties and methods available to the template
    ...
    startGame: function() {
      ...
      self.playing(true);
      localStorage.setItem("mastermind", "Computer 2");
      localStorage.setItem("mastermind-color", "#0F0");
      localStorage.setItem("mastermind-background", "rgba(0, 0, 0, 0.6)");
    },
    ...
    submitRow: function() {
      ...
      self.playingSequence.removeAll();
      if (matches.correct != params.win) {
        self.selectedIndex(0);
        if (matches.correct + matches.misplaced < params.win/(4/3)) {
          if (matches.correct < params.win/2) {
            score = 0;
          } else {
            score = 1;
          }
        } else if (matches.correct + matches.misplaced < params.win) {
          if (matches.correct < params.win/2) {
            score = 1;
          } else {
            score = 2;
      ...
      var xhttp = new XMLHttpRequest();
			xhttp.open("GET", "uhh1.php?mastermind=" + score, true);
			xhttp.send();
    }
  });
